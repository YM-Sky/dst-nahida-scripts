...
function FN.CreateAoeTargeting(inst, executeFn, data)
    ...
    print("CreateAoeTargeting")

    -- 确保 DEGREES 常量可用
    local DEGREES_LOCAL = DEGREES or (PI / 180)
    ...
    local spellMinDis = Utils.GetVal(data, "spellMinDis",15)                        --施法最大距离（实际上是最大距离，不是最小距离）
    local spellMinDisSq
    if spellMinDis ~= nil then
        spellMinDisSq = spellMinDis * spellMinDis
    else
        spellMinDis = math.huge
    ...
    -- 验证指示器是否成功创建
    if not r then
        print("CreateAoeTargeting: 无法创建指示器预制体:", reticulePrefab)
        return function() end -- 返回空的结束函数
    end

    print("CreateAoeTargeting: 指示器预制体创建成功:", reticulePrefab)

    -- 创建施法距离指示器（圈圈）
    local range_indicator = nil
    if spellMinDis and spellMinDis > 0 and spellMinDis ~= math.huge then
        -- 尝试使用不同的预制体来获得更好的虚线效果
        local reticule_prefabs = {"reticuleaoe", "reticule", "reticuleaoehostiletarget"}

        for _, prefab_name in ipairs(reticule_prefabs) do
            range_indicator = SpawnPrefab(prefab_name)
            if range_indicator then
                print("CreateAoeTargeting: 成功创建指示器:", prefab_name)
                break
            end
        end
        ...
        if range_indicator then
            -- 设置为角色位置
            range_indicator.Transform:SetPosition(inst.Transform:GetWorldPosition())
            if range_indicator.AnimState then
                -- 设置为白色虚线效果，尝试不同的颜色和透明度
                range_indicator.AnimState:SetMultColour(1, 1, 1, 0.8) -- 白色，稍微不透明一些
                range_indicator.AnimState:PlayAnimation("idle", true)

                -- 根据施法距离设置合适的缩放
                -- 不同预制体可能有不同的基础大小，需要调整
                -- 根据实际测试，圈圈边缘距离约为10，缩放为4，所以基础半径应该是2.5
                local base_radius = 1.5 -- reticule 的基础半径
                if range_indicator.prefab == "reticuleaoe" then
                    base_radius = 2.5 -- 调整为正确的基础半径，使圈圈与施法距离8匹配
                elseif range_indicator.prefab == "reticuleaoehostiletarget" then
                    base_radius = 2.5 -- 与reticuleaoe类似
                end

                local scale = spellMinDis / base_radius
                range_indicator.AnimState:SetScale(scale, scale, scale)
                print("CreateAoeTargeting: 设置施法距离圈缩放:", scale, "施法距离:", spellMinDis, "预制体:", range_indicator.prefab)
            end
            range_indicator:Show()
            print("CreateAoeTargeting: 创建施法距离指示器，距离:", spellMinDis)
        else
            print("CreateAoeTargeting: 无法创建任何施法距离指示器")
        end
    end

    -- 确保指示器在正确的渲染层和颜色设置
    if r.AnimState then
        r.AnimState:SetLayer(LAYER_WORLD_BACKGROUND)
        r.AnimState:SetSortOrder(1)
        -- 设置指示器颜色 - 绿色表示可用
        r.AnimState:SetMultColour(1, 1, 1, 1)
        -- 播放默认动画
        r.AnimState:PlayAnimation("idle", true)
        print("CreateAoeTargeting: 设置指示器渲染层、颜色和动画")
    end

    local function UpdateRotate()
        if reticuleRotate and targetpos and r and r:IsValid() then
            local x, y, z = targetpos:Get()
            if x and y and z then
                r:ForceFacePoint(x, y, z)
            end
        end
    end
    local UpdatePosition = function(dt)
        if targetpos then
            if reticuleFollowMouse then
                local x, y, z = targetpos:Get()
                if x and y and z and r and r:IsValid() and r.Transform then
                    r.Transform:SetPosition(x, y, z)
                    -- 减少调试输出频率
                    -- print("CreateAoeTargeting: 更新指示器位置到:", x, y, z)
                end
            else
                if inst and inst:IsValid() and inst.Transform then
                    ...
                    if x and y and z and r and r:IsValid() and r.Transform then
                        r.Transform:SetPosition(x, y, z)
                        -- print("CreateAoeTargeting: 更新指示器位置到角色位置:", x, y, z)
                    end
                end
            end
        else
            print("CreateAoeTargeting: 警告 - targetpos 为 nil，无法更新位置")
        end

        -- 更新施法距离圈的位置，让它跟随角色
        if range_indicator and range_indicator:IsValid() and inst and inst:IsValid() and inst.Transform then
            local x, y, z = inst.Transform:GetWorldPosition()
            if x and y and z then
                range_indicator.Transform:SetPosition(x, y, z)
            else
                print("CreateAoeTargeting: 警告 - 无法获取角色位置")
            end
        end
    end
    local endFn
    endFn = function()
        print("CreateAoeTargeting: 清理指示器")
        ...
        -- 清理施法距离指示器
        if range_indicator and range_indicator:IsValid() then
            range_indicator:Remove()
        end

        -- 清理玩家的指示器引用
        if inst and inst._aoe_targeting_end_fn == endFn then
            inst._aoe_targeting_end_fn = nil
        end
    end

    local function clickFn(button, down, x, y)
        if (isRight and button == MOUSEBUTTON_RIGHT or button == MOUSEBUTTON_LEFT) and down then
            -- 在点击时重新获取鼠标位置，确保targetpos是最新的
            local x1, y1, z1 = TheSim:ProjectScreenPos(x, y)
            local click_pos = x1 ~= nil and y1 ~= nil and z1 ~= nil and Vector3(x1, y1, z1) or nil

            if click_pos then
                if mouseTargetFn ~= nil then
                    targetpos = mouseTargetFn(inst, click_pos)
                else
                    targetpos = click_pos
                end
                print("CreateAoeTargeting: 点击时更新目标位置:", targetpos)
            end

            -- 确保有有效的目标位置
            if not targetpos then
                print("CreateAoeTargeting: 没有有效的目标位置")
                endFn()
                return
            end
            ...
            print("CreateAoeTargeting: 玩家位置:", playerPos)
            print("CreateAoeTargeting: 目标位置:", targetpos)

            -- 手动计算距离进行对比
            local dx = targetX - playerPos.x
            local dz = targetZ - playerPos.z
            local manual_distance = math.sqrt(dx*dx + dz*dz)

            -- 检测距离是否足够
            local disSq = inst:GetDistanceSqToPoint(targetpos:Get())
            local currentDis = math.sqrt(disSq)

            print("CreateAoeTargeting: 当前距离:", currentDis, "手动计算距离:", manual_distance, "最大施法距离:", spellMinDis)
            print("CreateAoeTargeting: 距离差异:", math.abs(currentDis - manual_distance))

            if currentDis > spellMinDis then
                print("CreateAoeTargeting: 距离太远，需要移动到施法范围内")
                local pos = inst:GetPosition()
                local targetX, targetY, targetZ = targetpos:Get()
                local dx, dz = targetX - pos.x, targetZ - pos.z
                local dis = math.sqrt(dx*dx + dz*dz)

                -- 计算移动到施法范围边缘的位置（朝目标方向移动，但保持在施法距离内）
                local moveRatio = (spellMinDis - 0.5) / dis -- 移动到距离目标 spellMinDis-0.5 的位置
                local moveToX = pos.x + dx * moveRatio
                local moveToZ = pos.z + dz * moveRatio
                local moveToPos = Vector3(moveToX, pos.y, moveToZ)

                print("CreateAoeTargeting: 移动到位置:", moveToPos, "距离目标:", math.sqrt((targetX - moveToX)^2 + (targetZ - moveToZ)^2))
                if inst.components.locomotor then
                    inst.components.locomotor:GoToPoint(moveToPos, nil, true)
                end
                -- 距离不够时不结束指示器，让玩家可以重新选择位置
            else
                print("CreateAoeTargeting: 距离足够，执行技能")
                if pingPrefab then
    ...
    if mouseEnabled and not TheInput:ControllerAttached() then
        r.followhandler = TheInput:AddMoveHandler(function(x, y)
            ...
            if mouseTargetFn ~= nil then
                targetpos = mouseTargetFn(inst, pos)
            else
                targetpos = pos
            end
            if targetpos then
                -- 添加调试信息：显示鼠标移动时的距离
                local currentDis = inst:GetDistanceSqToPoint(targetpos:Get())
                currentDis = math.sqrt(currentDis)
                if currentDis > spellMinDis then
                    print("CreateAoeTargeting: 鼠标移动 - 距离:", currentDis, "超出范围:", spellMinDis)
                end
                UpdateRotate()
            end
        end)
        ...
        if targetFn ~= nil then
            targetpos = targetFn(inst)
        else
            -- 如果没有提供 targetFn，默认使用角色前方位置
            local x, y, z = inst.Transform:GetWorldPosition()
            local angle = inst.Transform:GetRotation() * DEGREES_LOCAL
            targetpos = Vector3(x + 2 * math.cos(angle), y, z - 2 * math.sin(angle))
        end
    end
    -- 确保有有效的目标位置
    if not targetpos then
        print("CreateAoeTargeting: 警告 - 没有设置有效的目标位置，使用默认位置")
        local x, y, z = inst.Transform:GetWorldPosition()
        local angle = inst.Transform:GetRotation() * DEGREES_LOCAL
        targetpos = Vector3(x + 2 * math.cos(angle), y, z - 2 * math.sin(angle))
    end

    print("CreateAoeTargeting: 目标位置设置为:", targetpos)

    -- 添加更多调试信息
    print("CreateAoeTargeting: 指示器是否有效:", r and r:IsValid())
    print("CreateAoeTargeting: 指示器是否可见:", r and r.entity and r.entity:IsVisible())
    print("CreateAoeTargeting: 鼠标是否启用:", mouseEnabled)
    print("CreateAoeTargeting: 指示器跟随鼠标:", reticuleFollowMouse)

    UpdatePosition()
    UpdateRotate()

    -- 强制显示指示器
    if r and r.entity then
        r.entity:SetCanSleep(false)
        r:Show()
        print("CreateAoeTargeting: 强制显示指示器")
    end
    ...
    print("CreateAoeTargeting: 设置完成，返回结束函数")
    return endFn
end
