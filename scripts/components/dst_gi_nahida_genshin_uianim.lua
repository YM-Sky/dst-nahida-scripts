...
local Genshin_UIAnim = Class(function(self, inst)
    self.inst = inst
    self.update_while_paused = true
end)

function Genshin_UIAnim:UpdateWhilePaused(update_while_paused)
    self.update_while_paused = update_while_paused
end

function Genshin_UIAnim:FinishCurrentTint()
...
function Genshin_UIAnim:CancelTintTo( run_complete_fn )
	self.tint_t = nil
...
function Genshin_UIAnim:TintTo(start, dest, duration, whendone)
...
function Genshin_UIAnim:FinishCurrentScale()
    if not self.inst or not self.inst:IsValid() then
        -- sometimes the ent becomes invalid during a "finished" callback, but this gets run anyways.
        return
    end
    ...
    self.inst.UITransform:SetScale(sx >= 0 and val.x or -val.x, sy >= 0 and val.y or -val.y, sz >= 0 and val.z or -val.z)
...
function Genshin_UIAnim:CancelScaleTo( run_complete_fn )
	self.scale_t = nil
...
function Genshin_UIAnim:ScaleTo(start, dest, duration, whendone)
...
function Genshin_UIAnim:CancelMoveTo( run_complete_fn )
	self.pos_t = nil
...
function Genshin_UIAnim:MoveTo(start, dest, duration, whendone)
...
function Genshin_UIAnim:CancelRotateTo( run_complete_fn )
	self.rot_t = nil
...
function Genshin_UIAnim:RotateTo(start, dest, duration, whendone, infinite )
...
function Genshin_UIAnim:OnWallUpdate(dt)
    ...
    if self.scale_t then
        ...
        if sx and sy and sz then
			...
			if self.scale_t < self.scale_duration then
				local val_x = easing.outCubic( self.scale_t, self.scale_start.x, self.scale_dest.x - self.scale_start.x, self.scale_duration)
                local val_y = easing.outCubic( self.scale_t, self.scale_start.y, self.scale_dest.y - self.scale_start.y, self.scale_duration)
                local val_z = easing.outCubic( self.scale_t, self.scale_start.z, self.scale_dest.z - self.scale_start.z, self.scale_duration)
			    self.inst.UITransform:SetScale(sx >= 0 and val_x or -val_x, sy >= 0 and val_y or -val_y, sz >= 0 and val_z or -val_z)
            else
                self:FinishCurrentScale()
            end
		end
