...
---在指定的圆形区域内生成分布均匀的随机点集，泊松盘算法
---@param center Vector3 圆心位置
...
---@param isSatisfy boolean 当指定区域生成的点数不满足时是否允许插入几个随机点凑数，默认false
---@return table{} 二维数组，{{x,z},{x,z},{x,z},...}
function FN.GetRandomCircleLocations(center, radius, count, minDis, k, isSatisfy)
    k = k or 30
    isSatisfy = isSatisfy or false

    -- 网格大小，确保每个网格最多包含一个点
    local cellSize = minDis / math.sqrt(2)
    local gridWidth = math.ceil(2 * radius / cellSize)
    ...
    local activeList = {}  -- 活跃点列表
    local samplePoints = {} -- 最终结果
    local pi2 = math.pi * 2
    ...
    local function worldToGrid(x, z)
        local gridX = math.floor((x - center.x + radius) / cellSize) + 1
        local gridZ = math.floor((z - center.z + radius) / cellSize) + 1
        return gridX, gridZ
    end

    -- 辅助函数：检查网格坐标是否有效
    local function isValidGrid(gridX, gridZ)
        return gridX >= 1 and gridX <= gridWidth and gridZ >= 1 and gridZ <= gridHeight
    end

    -- 辅助函数：检查新点是否与现有点距离足够
    local function isValidDistance(x, z)
        local gridX, gridZ = worldToGrid(x, z)
        ...
        for i = -2, 2 do
            for j = -2, 2 do
                ...
                if isValidGrid(checkX, checkZ) and grid[checkZ][checkX] then
                    local existingPoint = grid[checkZ][checkX]
                    local dx = x - existingPoint.x
                    local dz = z - existingPoint.z
                    if dx * dx + dz * dz < minDis * minDis then
                        return false
                    end
                end
            end
    ...
    -- 辅助函数：在圆内生成随机点
    local function randomPointInCircle()
    ...
    -- 辅助函数：在指定点周围的圆环内生成随机点
    local function randomPointAroundPoint(refX, refZ)
        local angle = math.random() * pi2
        local r = minDis * (1 + math.random()) -- 距离在[minDis, 2*minDis]之间
        local x = refX + r * math.cos(angle)
        local z = refZ + r * math.sin(angle)
        return x, z
    end

    -- 生成第一个随机点
    local firstX, firstZ = randomPointInCircle()
    local gridX, gridZ = worldToGrid(firstX, firstZ)

    local firstPoint = {x = firstX, z = firstZ}
    grid[gridZ][gridX] = firstPoint
    table.insert(activeList, firstPoint)
    table.insert(samplePoints, {x = firstX, z = firstZ})

    -- 主循环：从活跃列表中选择点，尝试在其周围生成新点
    while #activeList > 0 and (not count or #samplePoints < count) do
        -- 随机选择一个活跃点
        local randomIndex = math.random(#activeList)
        local activePoint = activeList[randomIndex]
        local foundNewPoint = false

        -- 尝试k次在该点周围生成新点
        for attempt = 1, k do
            local newX, newZ = randomPointAroundPoint(activePoint.x, activePoint.z)

            -- 检查新点是否在圆内且距离合适
            if isInCircle(newX, newZ) and isValidDistance(newX, newZ) then
                local newGridX, newGridZ = worldToGrid(newX, newZ)

                if isValidGrid(newGridX, newGridZ) then
                    local newPoint = {x = newX, z = newZ}
                    grid[newGridZ][newGridX] = newPoint
                    table.insert(activeList, newPoint)
                    table.insert(samplePoints, {x = newX, z = newZ})
                    foundNewPoint = true
                    break
                end
            end
        ...
        -- 如果没有找到新点，从活跃列表中移除该点
        if not foundNewPoint then
            table.remove(activeList, randomIndex)
        end
    end

    -- 如果需要补充点数且允许插入随机点
    if count and #samplePoints < count and isSatisfy then
        local attempts = 0
        local maxAttempts = (count - #samplePoints) * 10 -- 限制尝试次数避免无限循环

        while #samplePoints < count and attempts < maxAttempts do
            attempts = attempts + 1
            local newX, newZ = randomPointInCircle()

            -- 检查是否与现有点距离足够（使用较小的最小距离）
            local validDistance = true
            local reducedMinDis = minDis * 0.7 -- 减少30%的最小距离要求

            for _, point in ipairs(samplePoints) do
                local dx = newX - point.x
                local dz = newZ - point.z
                if dx * dx + dz * dz < reducedMinDis * reducedMinDis then
                    validDistance = false
                    break
                end
            end
