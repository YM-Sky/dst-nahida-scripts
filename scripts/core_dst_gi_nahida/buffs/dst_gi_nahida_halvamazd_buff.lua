---
--- dst_gi_nahida_halvamazd_buff.lua
--- Description: 哈瓦玛玛兹buff
--- Author: 没有小钱钱
--- Date: 2025/5/4 18:07
---
...
local function addBuff(target)
    if target.components.health then
        target.components.health.externalabsorbmodifiers:SetModifier(target, 0.45, "dst_gi_nahida_halvamazd_buff") -- 受到伤害减少45%
    end
...
local function removeBuff(target)
    -- 提升防御
    if target.components.health then
        target.components.health.externalabsorbmodifiers:RemoveModifier(target,"dst_gi_nahida_halvamazd_buff")
    end
end

return {
    id = TUNING.MOD_ID .. "_halvamazd_buff", -- 唯一标识符，哈瓦玛玛兹buff
    ...
    buff_string = STRINGS.MOD_DST_GI_NAHIDA.NAHIDA_HALVAMAZD_BUFF  -- 增益效果的名称
}